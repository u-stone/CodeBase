...
static inline char *av_ts_make_string(int64_t ts)
{
    if (ts == AV_NOPTS_VALUE) _snprintf(buf, AV_TS_MAX_STRING_SIZE, "NOPTS");
    else                      _snprintf(buf, AV_TS_MAX_STRING_SIZE, "%d", ts);
    return buf;
}
...
#define av_ts2str(ts) av_ts_make_string(ts)
 ...
 * @return the buffer in input
 */
static inline char *av_ts_make_time_string(int64_t ts, AVRational *tb)
{
    if (ts == AV_NOPTS_VALUE) _snprintf(buf, AV_TS_MAX_STRING_SIZE, "NOPTS");
