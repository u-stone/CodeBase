...
void CErrorLookup_MFCDlg::OnBnClickedbtnlookup()
{
	...
	while (len--){
		if (strErrNo[len] > '9' || strErrNo[len] < '0'){
			GetDlgItem(IDC_edtErrorMsg)->SetWindowText(_T("输入有误..."));
			return ;
		}
	}
	...
	{
		// Handle the error.
		GetDlgItem(IDC_edtErrorMsg)->SetWindowText(_T("没找到..."));
		return ;
	}
